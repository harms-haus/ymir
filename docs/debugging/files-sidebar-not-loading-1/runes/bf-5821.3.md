# bf-5821.3 — Add error state and retry UI in AllFilesTab
...
2. Update ErrorResponse handler (line 135-140):
   - Extract error message from `_message.payload` (check `message.code` and `message.message` fields)
...
4. Add error render path between loading and "no files" checks (around line 269):
   ```tsx
   if (error) {
     return (
   ...
   ```
5. Add `handleRetry` callback:
   - `setError(null)`
   - `setIsLoading(true)`
   - Resend `FileList` request with `activeWorktree.id`
   - Set `pendingFileListWorktreeId`
   - Set new 15s timeout
6. Clear `error` in the `activeWorktree` change effect (line 126-130)
7. Clear `error` in the timeout handler (line 192-199) — or alternatively, the timeout should also set an error

## Success criteria
- When `FILE_LIST_ERROR` arrives, user sees error message with retry button
- Retry button clears error, shows loading, re-fetches file list
