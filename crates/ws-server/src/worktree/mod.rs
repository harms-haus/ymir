...
fn git_worktree_name(workspace_id: &str, branch_name: &str) -> String {
    format!(
        "{}_{}",
        workspace_id.split('-').next().unwrap_or(workspace_id),
        branch_name.replace('/', "_")
    )
}

fn parse_worktree_created_at(created_at: &str) -> Result<u64> {
    Ok(chrono::DateTime::parse_from_rfc3339(created_at)
        .with_context(|| format!("Invalid worktree created_at in database: {created_at}"))?
        .timestamp() as u64)
}

fn remove_orphaned_git_worktree_metadata(repo_root: &Path, worktree_name: &str) -> Result<()> {
    let metadata_dir = repo_root.join(".git").join("worktrees").join(worktree_name);
    if metadata_dir.exists() {
        std::fs::remove_dir_all(&metadata_dir).with_context(|| {
            format!(
                "Failed to remove orphaned git worktree metadata directory: {}",
                metadata_dir.display()
            )
        })?;
        info!(
            "Removed orphaned git worktree metadata: {}",
            metadata_dir.display()
        );
    }
...
pub async fn create(state: Arc<AppState>, msg: WorktreeCreate) -> Result<WorktreeCreated> {
    ...
    let repo = Repository::open(&workspace.root_path).context("Failed to open git repository")?;
    ...
    let worktree_name = git_worktree_name(&msg.workspace_id.to_string(), &msg.branch_name);
    let force = false;

    let git_worktree_dir = Path::new(&workspace.root_path)
        .join(".git")
        .join("worktrees")
        .join(&worktree_name);
    if git_worktree_dir.exists() {
        if !force {
            anyhow::bail!(
                "Git worktree metadata directory already exists: {}",
...
pub async fn delete(state: Arc<AppState>, msg: WorktreeDelete) -> Result<WorktreeDeleted> {
    ...
    // Open the git repository
    let repo = Repository::open(&workspace.root_path).context("Failed to open git repository")?;
    let worktree_name = git_worktree_name(&worktree.workspace_id, &worktree.branch_name);
    ...
    match repo.find_worktree(&worktree_name) {
        Ok(git_worktree) => {
            ...
            if let Err(err) = git_worktree.prune(Some(&mut prune_options)) {
                warn!(
                    "Failed to prune git worktree metadata for {}: {}",
                    worktree_name, err
                );
            }
        }
        Err(err) => {
            warn!(
                "Failed to find git worktree metadata for {}: {}",
                worktree_name, err
            );
        }
    }
    remove_orphaned_git_worktree_metadata(Path::new(&workspace.root_path), &worktree_name)?;

    // Delete worktree from database
    let deleted = state
        .db
        .delete_worktree(&msg.worktree_id.to_string())
        .await
...
pub async fn status(state: Arc<AppState>, worktree_id: Uuid) -> Result<WorktreeStatus> {
    let worktree = state
        .db
        .get_worktree(&worktree_id.to_string())
        .await
        .context("Failed to fetch worktree from database")?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_create_worktree_fails_when_git_metadata_already_exists() {
        let (state, temp_dir) = create_test_state().await;
        let (workspace_id, root_path) = create_test_workspace(&state, &temp_dir).await;

        let branch_name = format!(
            "feature/existing-metadata-{}",
            Uuid::new_v4().to_string().split('-').next().unwrap()
        );
        let worktree_name = format!(
            "{}_{}",
            workspace_id.to_string().split('-').next().unwrap(),
            branch_name.replace('/', "_")
        );
        let metadata_dir = Path::new(&root_path)
            .join(".git")
            .join("worktrees")
            .join(&worktree_name);
        std::fs::create_dir_all(&metadata_dir).expect("Failed to create metadata dir");
        let sentinel = metadata_dir.join("sentinel");
        std::fs::write(&sentinel, "keep me").expect("Failed to write sentinel file");
        ...
        assert!(
            result.is_err(),
            "Create should fail when metadata already exists"
        );
        assert!(
            sentinel.exists(),
            "Existing metadata should not be deleted without force"
        );
    }

    #[tokio::test]
    async fn test_delete_worktree_removes_git_metadata() {
        let (state, temp_dir) = create_test_state().await;
        let (workspace_id, root_path) = create_test_workspace(&state, &temp_dir).await;
        ...
        assert!(
            metadata_dir.exists(),
            "Expected git metadata to exist after create"
        );

        let delete_msg = WorktreeDelete {
            worktree_id: created.worktree.id,
        };
        delete(state.clone(), delete_msg)
            .await
            .expect("Failed to delete worktree");

        let repo = Repository::open(&root_path).expect("Failed to reopen repo");
        assert!(
            repo.find_worktree(&worktree_name).is_err(),
            "Worktree metadata should be pruned"
        );
        assert!(
            !metadata_dir.exists(),
            "Metadata directory should be removed after delete"
        );
    }

    #[tokio::test]
    async fn test_list_worktrees_errors_on_invalid_uuid() {
        let (state, temp_dir) = create_test_state().await;
        let (workspace_id, root_path) = create_test_workspace(&state, &temp_dir).await;

        state
            .db
            .create_worktree(&DbWorktree {
                id: "not-a-uuid".to_string(),
                workspace_id: workspace_id.to_string(),
                branch_name: "feature/bad-id".to_string(),
                path: root_path,
                status: "active".to_string(),
                created_at: chrono::Utc::now().to_rfc3339(),
            })
        ...
        let result = list(state.clone(), WorktreeList { workspace_id }).await;
        assert!(result.is_err(), "List should surface invalid UUID data");
    }
    ...
    async fn test_list_worktrees() {
        ...
        let worktrees = list(state.clone(), list_msg)
            .await
